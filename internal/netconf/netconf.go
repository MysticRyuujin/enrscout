...
func (n *Network) IsCurrentForkAt(raw string, at time.Time) bool {
	got, ok := parseHash4(raw)
	return ok && got == n.CurrentForkIDAt(at).Hash
}

// parseHash4 accepts a 4-byte fork hash or digest as hex, with optional 0x and surrounding space.
func parseHash4(raw string) ([4]byte, bool) {
	var out [4]byte
	b, err := hex.DecodeString(strings.TrimPrefix(strings.ToLower(strings.TrimSpace(raw)), "0x"))
...
// ParseNetworkList parses a comma-separated list of registered networks, skipping empty entries
// and rejecting unknown or repeated names.
func ParseNetworkList(csv string) ([]string, error) {
	var out []string
	...
	for _, name := range strings.Split(csv, ",") {
		...
		if seen[name] {
			return nil, fmt.Errorf("duplicate network %q", name)
		}
		seen[name] = true
		out = append(out, name)
	}
	if len(out) == 0 {
		return nil, errors.New("must list at least one network")
	}
	return out, nil
}
...
func Bootnodes(name string) ([]*enode.Node, error) {
	n, err := Get(name)
	if err != nil || len(n.bootnodes) == 0 {
		return nil, fmt.Errorf("no bootnodes for network %q", name)
	}
	nodes := make([]*enode.Node, 0, len(n.bootnodes))
	for _, u := range n.bootnodes {
		node, err := enode.Parse(enode.ValidSchemes, u)
		if err != nil {
...
// CLBootnodes skips an unparseable record rather than failing, unlike Bootnodes: consensus lists
// are copied from upstream configs that occasionally ship a record go-ethereum rejects.
func CLBootnodes(name string) ([]*enode.Node, error) {
	n, err := Get(name)
	if err != nil || len(n.clBootnodes) == 0 {
		return nil, fmt.Errorf("no consensus bootnodes for network %q", name)
	}
	nodes := make([]*enode.Node, 0, len(n.clBootnodes))
	for _, u := range n.clBootnodes {
		if node, err := enode.Parse(enode.ValidSchemes, u); err == nil {
			nodes = append(nodes, node)
		}
	}
	if len(nodes) == 0 {
