...
provides a live network-membership claim it is published with its enode URL and an empty `enr` field -
ENRScout never fabricates an ENR.
...
history from a single vantage point - repeated sightings are not independent
evidence - and is not confidence, sync status, or application-protocol
reachability. At the retention limit, lower-value records (score-zero fallback
...
consensus Status for consensus nodes) - the peer's network ID and genesis, or its fork
...
- **Discovery-only** - the node was found over discv4/discv5 but advertises no dialable
  ...
  explorer; it is simply not directly connectable.
